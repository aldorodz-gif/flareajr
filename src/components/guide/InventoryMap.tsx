...
const flareIcon = L.divIcon({
  ...
  html: '<div style="width:16px;height:16px;border-radius:50%;background:linear-gradient(135deg,#DC2626,#f9a8d4);border:2px solid #fff;box-shadow:0 2px 6px rgba(251,146,60,.5)"></div>',
  iconSize: [16, 16],
  iconAnchor: [8, 8],
});
const selectedIcon = L.divIcon({
  className: 'flare-marker-selected',
  html: '<div style="width:22px;height:22px;border-radius:50%;background:#0e1e3a;border:3px solid #DC2626;box-shadow:0 0 0 4px rgba(251,146,60,.25)"></div>',
  iconSize: [22, 22],
  iconAnchor: [11, 11],
});
  ...
  return (
    <div className="p-5 rounded-xl" style={{ background: '#fff', border: '1px solid rgba(14,30,58,.08)' }}>
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div>
          <Eyebrow gradient="linear-gradient(90deg, #D97FAA, #DC2626)">Core Inventory</Eyebrow>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-4">
        {/* Property list */}
        <div className="rounded-lg overflow-y-auto" style={{ maxHeight: 380, border: '1px solid rgba(14,30,58,.08)', background: '#FFFFFF' }}>
          {visible.length === 0 ? (
      ...
      {selected && (
        <div className="mt-5 p-4 rounded-lg" style={{ background: '#FFFFFF', border: '1px solid rgba(14,30,58,.08)' }}>
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            <div>
              <Eyebrow gradient="linear-gradient(90deg, #DC2626, #f9a8d4)">Local Verticals — Who to Prospect Nearby</Eyebrow>
          ...
          {!vLoading && verticals && (
            <>
              <div className="mb-4 p-3 rounded" style={{ background: 'rgba(251,146,60,.08)', borderLeft: '3px solid #DC2626' }}>
                <div className="text-[10px] font-bold uppercase tracking-wider mb-1" style={{ color: '#DC2626' }}>Angle</div>
                ...
                {verticals.nearby_targets.map((t, i) => (
                  <div key={i} className="p-3 rounded" style={{ background: '#fff', border: '1px solid rgba(14,30,58,.08)' }}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[11px]">
                      <div>
                        <span className="font-bold" style={{ color: '#DC2626' }}>Target: </span>
                        <span style={{ color: '#0e1e3a' }}>{t.target_titles.join(', ') || 'Recommended contacts vary by account'}</span>
                      </div>
                      <div>
                        <span className="font-bold" style={{ color: '#DC2626' }}>Signals: </span>
