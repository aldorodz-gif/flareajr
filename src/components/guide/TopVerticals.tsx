        ...
        {data.map((row, i) => (
          <div key={row.vertical}>
            <div className="flex items-baseline justify-between mb-1">
              ...
              <span className="text-[12px] font-bold tabular-nums" style={{ color: '#DC2626' }}>{row.share_pct}%</span>
            </div>
            <div className="h-1.5 rounded-full overflow-hidden mb-1" style={{ background: 'rgba(14,30,58,.06)' }}>
              <div
                className="h-full rounded-full"
                style={{ width: `${row.share_pct}%`, background: 'linear-gradient(90deg, #DC2626, #f9a8d4)' }}
              />
            </div>
