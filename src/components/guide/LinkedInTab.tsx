    ...
    <main className="max-w-5xl mx-auto px-4 md:px-8 py-8 space-y-8">
      ...
      >
        <div className="space-y-4">
          {/* Vertical */}
          <div>
            <label className="block text-xs font-semibold mb-1.5" style={{ color: '#DC2626' }}>Target Vertical</label>
            <div className="flex flex-wrap gap-2">
              {VERTICALS.map(v => (
                <button
                  ...
                  style={{
                    background: vertical === v ? 'linear-gradient(135deg,#DC2626,#db2777)' : 'rgba(0,0,0,.05)',
                    color: vertical === v ? '#fff' : '#475569',
                    border: `1px solid ${vertical === v ? '#DC2626' : 'rgba(0,0,0,.12)'}`,
                  }}
                >
                  {v}
                </button>
          ...
          {/* Signal */}
          <div>
            <label className="block text-xs font-semibold mb-1.5" style={{ color: '#DC2626' }}>Signal You've Spotted</label>
            <input
              value={signal}
          ...
          {/* Goal (optional) */}
          <div>
            <label className="block text-xs font-semibold mb-1.5" style={{ color: '#DC2626' }}>Goal <span className="text-gray-400">(optional)</span></label>
            <input
              value={goal}
            ...
            {posts.map((post, i) => {
              ...
              return (
                <div
                  ...
                  style={{ background: '#FFFFFF', border: '1px solid rgba(251,146,60,.12)' }}
                >
                  {/* Header */}
                  <div className="px-4 py-3 flex items-center justify-between" style={{ background: 'linear-gradient(135deg,#1E293B,#1a1f3d)', borderBottom: '1px solid rgba(251,146,60,.15)' }}>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold" style={{ color: '#DC2626' }}>POST {post.number}</span>
                      <span className="text-xs font-medium text-white/60">— {post.type}</span>
                    </div>
                    <span className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: 'rgba(251,146,60,.15)', color: '#DC2626' }}>
                      🕐 {post.best_time}
                    </span>
                  </div>
                  ...
                  <div className="p-4 space-y-3">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {post.hashtags.map((h, hi) => (
                        <span key={hi} className="text-[11px] px-2 py-0.5 rounded-full" style={{ background: 'rgba(99,102,241,.1)', color: '#0EA5E9' }}>
                          {h.startsWith('#') ? h : `#${h}`}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="rounded-lg p-2.5" style={{ background: 'rgba(99,102,241,.05)' }}>
                      <span className="text-[10px] font-bold block mb-0.5" style={{ color: '#0EA5E9' }}>STRATEGIC INTENT</span>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        color: copiedIdx === i ? '#22c55e' : '#DC2626',
                      }}
                    >
